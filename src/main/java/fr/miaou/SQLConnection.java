...
public class SQLConnection {
    ...
    public void envoyerFiche(FicheFrais fiche) {
        ...
        try {
            statement.execute("UPDATE fdrm SET fdrm_reception = '" + date + "' WHERE fk_ag = '" + fiche.getAg()
                    + "' AND fdrm_mois = '" + fiche.getFdrmMois() + "' ");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public void validerFiche(FicheFrais fiche) {
        ...
        try {
            statement.execute("UPDATE fdrm SET fdrm_validation = '" + date + "' WHERE fk_ag = '" + fiche.getAg()
                    + "' AND fdrm_mois = '" + fiche.getFdrmMois() + "' ");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void payerFiche(FicheFrais fiche) {
        ...
        try {
            statement.execute("UPDATE fdrm SET fdrm_paiement = '" + date + "' WHERE fk_ag = '" + fiche.getAg()
                    + "' AND fdrm_mois = '" + fiche.getFdrmMois() + "' ");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void rembourserFiche(FicheFrais fiche) {
        ...
        try {
            statement.execute("UPDATE fdrm SET fdrm_remboursement = '" + date + "' WHERE fk_ag = '" + fiche.getAg()
                    + "' AND fdrm_mois = '" + fiche.getFdrmMois() + "' ");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void initialiser(ArrayList<Agent> agents, ArrayList<FicheFrais> fiches, ArrayList<Frais> frais,
            ArrayList<TypeRefus> typeRefus, ArrayList<Vehicule> vehicules) {
        try {
            statement = con.createStatement();

            if (DATE.getMonthValue() == 1) {
                statement.execute("UPDATE fdrm SET fdrm_reception = '" + DATE + "' WHERE fdrm_mois = " + 12
                        + " AND fdrm_reception IS NULL;");
            } else {
                statement.execute("UPDATE fdrm SET fdrm_reception = '" + DATE + "' WHERE fdrm_mois = "
                        + (DATE.getMonthValue() - 1) + " AND fdrm_reception IS NULL;");
            }

            Statement statementFrais = con.createStatement();

            ResultSet resultFrais = statementFrais.executeQuery("SELECT * FROM frais");
            while (resultFrais.next()) {
                frais.add(new Frais(resultFrais.getInt("fr_id"), resultFrais.getString("fr_libelle_libre"),
                        resultFrais.getObject("fr_date", LocalDate.class), resultFrais.getInt("fr_quantite"),
                        resultFrais.getDouble("fr_montant"), resultFrais.getInt("fr_status"),
                        resultFrais.getInt("fk_tre"),
                        resultFrais.getInt("fk_mfr"), resultFrais.getInt("fk_fdrm_ag"),
                        resultFrais.getInt("fk_fdrm_mois")));
            }

            Statement statementFdrm = con.createStatement();

            ResultSet resultFdrm = statementFdrm.executeQuery("SELECT * FROM fdrm");

            while (resultFdrm.next()) {
                fiches.add(new FicheFrais(resultFdrm.getInt("fk_ag"),
                        resultFdrm.getInt("fdrm_mois"),
                        resultFdrm.getObject("FDRM_reception", LocalDate.class),
                        resultFdrm.getObject("FDRM_validation", LocalDate.class),
                        resultFdrm.getObject("FDRM_paiement", LocalDate.class),
                        resultFdrm.getObject("FDRM_remboursement", LocalDate.class),
                        resultFdrm.getInt("nbrj_conges")));
            }

            Statement statementAgent = con.createStatement();

            resultSet = statementAgent.executeQuery("SELECT * FROM agents");
            while (resultSet.next()) {
                agents.add(new Agent(resultSet.getInt("ag_matricule"), resultSet.getString("ag_nom"),
                        resultSet.getString("ag_password"), resultSet.getInt("fk_se"), resultSet.getInt("fk_ta"),
                        resultSet.getString("fk_ve")));
            }

            Statement statementTypeRefus = con.createStatement();

            resultSet = statementTypeRefus.executeQuery("SELECT * FROM type_refus");
            while (resultSet.next()) {
                typeRefus.add(new TypeRefus(resultSet.getInt("tre_id"), resultSet.getString("tre_libelle")));
            }

            Statement statementVehicules = con.createStatement();

            resultSet = statementVehicules.executeQuery("SELECT * FROM vehicules");
            while (resultSet.next()) {
                vehicules.add(new Vehicule(resultSet.getString("ve_immat"), resultSet.getString("ve_marque"),
                        resultSet.getString("ve_model"), resultSet.getInt("fk_fkm")));
            }
    ...
    public void createFrais(ArrayList<Frais> frais, int idAgentDonne, String libelle, int quantite, double prix,
            int mois, LocalDate date) {
        try {
            int id = 0;
            ...
            frais.add(newFrais);
            statement.execute(
                    "insert into frais(fr_libelle_libre, fr_date, fr_quantite, fr_montant, fr_taxe, fr_status, fk_fdrm_ag, fk_fdrm_mois) values ('"
                            + libelle + "', '" + date + "', " + quantite + ", " + prix + ", " + newFrais.getTaxe()
                            + ", 0, " + idAgentDonne + ", " + mois + ")");
    ...
    public void createFraisForfaitaire(ArrayList<Frais> frais, int idAgentDonne, int idMfr, int quantite, int mois) {
        try {
            ...
            String libelle = "";
            resultSet = statement
                    .executeQuery("SELECT mfr_libelle FROM miaou.modele_frais WHERE mfr_id='" + idMfr + "'");
            while (resultSet.next()) {
                libelle = resultSet.getString("mfr_libelle");
            }
            double prix = 0;
            resultSet = statement
                    .executeQuery("SELECT mfr_montant_unitaire FROM miaou.modele_frais WHERE mfr_id='" + idMfr + "'");
            while (resultSet.next()) {
            ...
            frais.add(newFrais);
            statement.execute(
                    "insert into frais(fr_libelle_libre, fr_date, fr_quantite, fr_montant, fr_taxe, fr_status, fk_mfr, fk_fdrm_ag, fk_fdrm_mois) values ('"
                            + libelle + "', '" + date + "', " + quantite + ", " + prix + ", " + newFrais.getTaxe()
                            + ", 0, " + idMfr + ", " + idAgentDonne + ", " + mois + ")");
    ...
    public void createFraisKm(ArrayList<Frais> frais, int idAgentDonne, int quantite, int mois) {
        try {
            ...
            double prix = 0;
            resultSet = statement
                    .executeQuery("SELECT fk_ve FROM miaou.agents WHERE ag_matricule='" + idAgentDonne + "'");
            while (resultSet.next()) {
            ...
            frais.add(newFrais);
            statement.execute(
                    "insert into frais(fr_libelle_libre, fr_quantite, fr_montant, fr_taxe, fr_status, fk_fdrm_ag, fk_fdrm_mois) values ('"
                            + libelle + "', " + quantite + ", " + prix + ", " + newFrais.getTaxe() + ", 0, "
                            + idAgentDonne + ", " + mois + ")");
    ...
    public void createVehicule(ArrayList<Vehicule> vehicules, String ve_immat, String ve_marque, String ve_model) {
        try {
            ...
            statement.execute("insert into vehicules(ve_immat, ve_marque, ve_model, fk_fkm) values ('" + ve_immat
                    + "', '" + ve_marque + "', '" + ve_model + "', 1)");
        } catch (SQLException e) {
    ...
    public void createFicheFrais(ArrayList<FicheFrais> fiches, int idAgentDonne, int mois) {
        try {
            ...
            System.out
                    .println("Erreur : Merci de vérifier si la fiche de frais du mois selectionné n'existe pas déjà.");
        }
    }
